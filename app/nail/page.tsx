...
const nailConditions = [
  {
    ...
    image: "/ony.jpg", // Specific image for Fungal
  ...
  },
  {
    ...
    image: "/pys.webp", // Specific image for Psoriasis
  ...
  },
  {
    ...
    image: "/infection.jpg", // Specific image for Bacterial
  ...
  },
  {
    ...
    image: "/onyc.webp", // Specific image for Separation
...
export default function NailTreatmentPage() {
  ...
  return (
    <main className="bg-white">
      ...
      <section className="relative pt-32 pb-20 lg:pt-48 lg:pb-32 bg-[#F9F9F7] overflow-hidden">
        <div className="max-w-7xl mx-auto px-6 md:px-10">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
            ...
            >
              ...
              <h1 className="text-3xl md:text-5xl lg:text-5xl font-light text-zinc-900 tracking-tight leading-[1.1]">
                Nail Disorders <br />
              ...
              <div className="flex flex-wrap gap-4 pt-4">
                <div className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-widest text-zinc-900 border-b-2 border-zinc-900 pb-1">
                  Dublin Clinic
                </div>
            ...
            >
              <div className="aspect-[6/5] rounded-[3rem] overflow-hidden shadow-2xl z-10 relative">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            {[
              ...
              <motion.div 
                ...
                viewport={{ once: true }}
                className="p-8 rounded-[2rem] bg-zinc-50 border border-zinc-100 space-y-4 hover:shadow-lg transition-all duration-500"
              >
      ...
      {/* SECTION 3: CONDITIONS TABS (Dynamic Images) */}
      <section className="py-24 bg-zinc-900 text-white overflow-hidden rounded-[4rem]">
        <div className="max-w-7xl mx-auto px-6 md:px-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
            <div className="space-y-12">
              <div className="space-y-4">
                <span className="text-[10px] font-bold uppercase tracking-[0.4em] text-zinc-500">Conditions We Treat</span>
                <h2 className="text-4xl md:text-5xl font-light tracking-tight text-white">Expertise in <span className="italic font-serif text-zinc-400">Pathology.</span></h2>
              </div>
              ...
              <div className="flex flex-col space-y-4">
                {nailConditions.map((condition) => (
                  <button
                    ...
                    className={`text-left px-8 py-6 rounded-3xl transition-all duration-500 border flex justify-between items-center group ${
                      activeTab.id === condition.id 
                      ? "bg-white border-white text-zinc-900 scale-[1.02]" 
                      : "bg-transparent border-white/10 text-zinc-500 hover:border-white/30"
                    }`}
                  >
                    <span className="text-xl font-medium tracking-tight">{condition.title}</span>
                    <ArrowRight className={`transition-transform duration-500 ${activeTab.id === condition.id ? "translate-x-0" : "-translate-x-4 opacity-0 group-hover:opacity-100"}`} />
                  </button>
                ))}
              </div>
            </div>
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  ...
                  <div className="space-y-6">
                    <h3 className="text-3xl font-light">{activeTab.subtitle}</h3>
                    <p className="text-zinc-400 text-lg font-light leading-relaxed">{activeTab.description}</p>
                      ...
                      <div className="space-y-4">
                        ...
                        <ul className="space-y-2">
                          {activeTab.symptoms.map(s => (
                            <li key={s} className="flex items-center gap-3 text-sm text-zinc-300 font-light">
                              <div className="w-1 h-1 rounded-full bg-zinc-500" /> {s}
                            </li>
                          ))}
                        </ul>
                      </div>
                      <div className="p-6 rounded-3xl bg-zinc-800 border border-white/5">
                        <span className="text-[10px] uppercase tracking-[0.3em] font-bold text-zinc-500 block mb-3">Approach</span>
                        <p className="text-sm text-zinc-400 font-light italic">{activeTab.approach}</p>
                      </div>
                    </div>
                  </div>
                </motion.div>
      ...
      <section className="py-24 md:py-32 px-6">
        <div className="max-w-7xl mx-auto bg-[#F9F9F7] rounded-[4rem] overflow-hidden shadow-sm">
            ...
            <div className="p-12 md:p-20 space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {[
                  ...
                  "Targets fungal organisms safely",
                  ...
                  <div key={item} className="flex items-center gap-3">
                    ...
                    <span className="text-sm text-zinc-800 font-medium tracking-tight">{item}</span>
                  </div>
                ))}
              </div>
            </div>
            <div className="h-full relative min-h-[500px]">
              <img src="/nail3.webp" alt="Lunula Laser Device" className="absolute inset-0 w-full h-full object-cover" />
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-24 px-6 max-w-4xl mx-auto">
        <div className="text-center space-y-12">
          <div className="space-y-4">
            ...
            <h2 className="text-3xl md:text-4xl font-light text-zinc-900">Important Medical Information</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 text-left">
            {[
              "Nail conditions may require laboratory confirmation prior to starting laser treatment.",
              "Multiple treatment sessions may be necessary depending on clinical severity.",
              "Results vary significantly based on individual nail growth rates and condition depth.",
              "A medical consultation is mandatory prior to receiving Lunula® Laser therapy."
            ].map((info, i) => (
          ...
          <div className="pt-8">
            <Link href="/#contact">
            <button className="bg-zinc-900 text-white px-12 py-5 rounded-full text-[11px] font-bold uppercase tracking-[0.3em] hover:bg-zinc-800 transition-all active:scale-95 shadow-xl shadow-zinc-200">
